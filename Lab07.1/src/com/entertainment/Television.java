...
public class Television implements Comparable<Television> {
	...
	private static final int MIN_DISALLOWED_CHANNEL = 500; // for parental
															// controls
	private static final int MAX_DISALLOWED_CHANNEL = 599; // for parental
															// controls
	...
	public void setVolume(int volume) throws IllegalArgumentException {
		if (volume >= MIN_VOLUME && volume <= MAX_VOLUME) {
			...
			throw new IllegalArgumentException(
					"Invalid volume: " + volume + ". " + "Allowed range: [" + MIN_VOLUME + "," + MAX_VOLUME + "].");
		}
	}
	...
	public void changeChannel(int channel) throws InvalidChannelException, ChannelDisallowedException {
		if (channel < MIN_CHANNEL || channel > MAX_CHANNEL) {
			throw new InvalidChannelException(
					"Invalid channel: " + channel + ". " + "Allowed range: [" + MIN_CHANNEL + "," + MAX_CHANNEL + "].");
		}
	...
	// parental control methods - JavaBeans-style accessor methods and "control"
	...
	public void setParentalControlEnabled(boolean parentalControl) {
		...
		if (isParentalControlEnabled()) {
			// reset channel if necessary (we might be on an "adult" channel
			// prior to enabling parental control)
